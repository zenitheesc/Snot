/* USER CODE BEGIN Header */
/**
  ******************************************************************************
  * @file           : main.h
  * @brief          : Header for main.c file.
  *                   This file contains the common defines of the application.
  ******************************************************************************
  * @attention
  *
  * Copyright (c) 2022 STMicroelectronics.
  * All rights reserved.
  *
  * This software is licensed under terms that can be found in the LICENSE file
  * in the root directory of this software component.
  * If no LICENSE file comes with this software, it is provided AS-IS.
  *
  ******************************************************************************
  */
/* USER CODE END Header */

/* Define to prevent recursive inclusion -------------------------------------*/
#ifndef __MAIN_H
#define __MAIN_H

#ifdef __cplusplus
extern "C" {
#endif

/* Includes ------------------------------------------------------------------*/
#include "stm32l4xx_hal.h"

/* Private includes ----------------------------------------------------------*/
/* USER CODE BEGIN Includes */

/* USER CODE END Includes */

/* Exported types ------------------------------------------------------------*/
/* USER CODE BEGIN ET */

/* USER CODE END ET */

/* Exported constants --------------------------------------------------------*/
/* USER CODE BEGIN EC */

/* USER CODE END EC */

/* Exported macro ------------------------------------------------------------*/
/* USER CODE BEGIN EM */

/* USER CODE END EM */

/* Exported functions prototypes ---------------------------------------------*/
void Error_Handler(void);

/* USER CODE BEGIN EFP */

/* USER CODE END EFP */

/* Private defines -----------------------------------------------------------*/
#define B1_Pin GPIO_PIN_13
#define B1_GPIO_Port GPIOC
#define B1_EXTI_IRQn EXTI15_10_IRQn
#define BUS_UART4_TX_Pin GPIO_PIN_0
#define BUS_UART4_TX_GPIO_Port GPIOA
#define BUS_UART4_RX_Pin GPIO_PIN_1
#define BUS_UART4_RX_GPIO_Port GPIOA
#define PICOBLADE_USART_TX_Pin GPIO_PIN_2
#define PICOBLADE_USART_TX_GPIO_Port GPIOA
#define PICOBLADE_USART_RX_Pin GPIO_PIN_3
#define PICOBLADE_USART_RX_GPIO_Port GPIOA
#define RESET_COMM_RX_Pin GPIO_PIN_12
#define RESET_COMM_RX_GPIO_Port GPIOB
#define SS_COMM_RX_Pin GPIO_PIN_13
#define SS_COMM_RX_GPIO_Port GPIOB
#define DIO0_COMM_RX_Pin GPIO_PIN_14
#define DIO0_COMM_RX_GPIO_Port GPIOB
#define DIO0_COMM_RX_EXTI_IRQn EXTI15_10_IRQn
#define LED1_Pin GPIO_PIN_6
#define LED1_GPIO_Port GPIOC
#define RESET_COMM_TX_Pin GPIO_PIN_9
#define RESET_COMM_TX_GPIO_Port GPIOC
#define SS_COMM_TX_Pin GPIO_PIN_8
#define SS_COMM_TX_GPIO_Port GPIOA
#define DIO0_COMM_TX_Pin GPIO_PIN_9
#define DIO0_COMM_TX_GPIO_Port GPIOA
#define DIO0_COMM_TX_EXTI_IRQn EXTI9_5_IRQn
#define TMS_Pin GPIO_PIN_13
#define TMS_GPIO_Port GPIOA
#define TCK_Pin GPIO_PIN_14
#define TCK_GPIO_Port GPIOA
#define SWO_Pin GPIO_PIN_3
#define SWO_GPIO_Port GPIOB
#define EXTERNAL_GPIO_Pin GPIO_PIN_4
#define EXTERNAL_GPIO_GPIO_Port GPIOB
/* USER CODE BEGIN Private defines */

/* USER CODE END Private defines */

#ifdef __cplusplus
}
#endif

#endif /* __MAIN_H */
